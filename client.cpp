...
void Client::DrawHUD() {
	...
	std::stringstream ss;
	ss << tfm::format(XOR("Hoti | fps: %i | ms: %i"), fps, ms);

	render::FontSize_t size = render::hud.size(ss.str().c_str());

	// background.
	render::rect_filled( m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, { 100, 45, 150, 20 } );
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::yellowgreen, XOR("[Hoti] "));
